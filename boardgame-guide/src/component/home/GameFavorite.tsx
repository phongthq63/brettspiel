...
export default function GameFavorite() {
    ...
    useEffect(() => {
        ...
        setFavoriteGames(mockPlayHistory);
    }, [])

    return (
        <Card>
            <CardHeader>
                <h3 className="text-xl font-bold">{t("gameFavorite.title")}</h3>
            </CardHeader>
            <CardBody>
                {favoriteGames.length === 0 ? (
                    <p className="text-gray-500 text-center py-4">{t("gameFavorite.noGames")}</p>
                ) : (
                        ...
                        {favoriteGames.map((game) => (
                            <li key={game.id} className="flex items-start gap-3 pb-3 border-b last:border-0">
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-xs">Cross North America by train, connecting iconic cities and completing tickets.</p>
